/**
 * @module   color.ts
 * @desc     Common palettes and simple color helpers
 * @category public
 *
...
/**
 * Basic RGB color with optional alpha
 */
...
/**
 * Basic color definition with integer value
 */
...
/**
 * Fully augmented color with all representations
 */
 ...
 */
export function rgb(r: number, g: number, b: number, a: number = 1.0): RGB {
	return { r, g, b, a };
}

/**
 * Convert r,g,b,a values to hex string
 */
export function hex(r: number, g: number, b: number, a: number = 1.0): string {
	return rgb2hex({ r, g, b, a });
}

/**
 * Convert r,g,b,a values to CSS string 'rgb(r,g,b)' or 'rgba(r,g,b,a)'
 */
export function css(r: number, g: number, b: number, a: number = 1.0): string {
...
/**
 * Convert {r,g,b,a} object to CSS string 'rgb(r,g,b)' or 'rgba(r,g,b,a)'
 */
...
/**
 * Convert {r,g,b,a} object to hex string '#RRGGBB' or '#RRGGBBAA'
 */
...
/**
 * Convert {r,g,b} values to grayscale value [0-1]
 * Uses standard luminance formula
 * @see https://en.wikipedia.org/wiki/Grayscale
 */
export function rgb2gray(rgb: RGB): number {
	return Math.round(rgb.r * 0.2126 + rgb.g * 0.7152 + rgb.b * 0.0722) / 255.0;
}

/**
 * Convert integer color (0xRRGGBB) to {r,g,b,a} object
 * Note: int is not a string but a number
 */
export function int2rgb(int: number): RGB {
	return {
		a: 1.0,
		r: (int >> 16) & 0xff,
		g: (int >> 8) & 0xff,
...
const _CSS2: ColorInt[] = [
	..._CSS1,
	{ int: 0xffa500, name: 'orange' }
];

const _CSS3: ColorInt[] = [
	..._CSS2,
...
const _CSS4: ColorInt[] = [
	..._CSS3,
	{ int: 0x663399, name: 'rebeccapurple' }
];

/**
 * Augments a color palette with all color representations
 */
function augment(pal: ColorInt[]): Color[] {
	return pal.map(el => {
		const rgb = int2rgb(el.int);
		const hex = rgb2hex(rgb);
		const cssStr = rgb2css(rgb);
		const v = rgb2gray(rgb);
		return { ...el, ...rgb, a: 1.0, v, hex, css: cssStr };
	});
}

/**
 * Converts a color array to a map keyed by color name
 */
function toMap(pal: Color[]): ColorMap {
	const out: ColorMap = {};
	pal.forEach(el => {
		out[el.name] = el;
	});
...
/**
 * CSS Level 4 color palette (map)
 * @example
 * const red = CSS4.red;
 * console.log(red.hex); // '#FF0000'
 * console.log(red.css); // 'rgb(255,0,0)'
 */
export const CSS4: ColorMap = toMap(augment(_CSS4));

/**
 * CSS Level 3 color palette (map)
 */
export const CSS3: ColorMap = toMap(augment(_CSS3));

/**
 * CSS Level 2 color palette (map)
 */
export const CSS2: ColorMap = toMap(augment(_CSS2));

/**
 * CSS Level 1 color palette (map)
 */
export const CSS1: ColorMap = toMap(augment(_CSS1));

/**
 * Commodore 64 color palette (array)
 */
export const C64: Color[] = augment(_C64);

/**
 * CGA color palette (array)
 */
export const CGA: Color[] = augment(_CGA);